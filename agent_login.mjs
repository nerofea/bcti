...
;(async () => {
  ...
  // Initialize the Noir ZK'd jr user  (set up and compile circuit)
  ...
  // Set up witness
  await import('./setupWitness.mjs')


  const profileBuffer = fs.readFileSync('./journalist_registry_compressed/alice.json.gz')
  const decompressed = zlib.gunzipSync(profileBuffer)
  const profile = JSON.parse(decompressed.toString())
  await verifyPassword(profile, './circuit.json', './vkey.json')
